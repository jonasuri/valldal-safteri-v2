...
export type PackingSlipDocumentData = {
    orderNumber: string;
    customerName: string;
...
function getLineCategory(line: PackingSlipLine) {
...
export default function PackingSlipDocument({
    ...
    const cancelledMissingLines = order.missingLinesDisposition === "cancelled";
    const hasMissingLines = !cancelledMissingLines && order.lines.some((line) => line.missingQuantity > 0);
    const hasSignature = Boolean(order.signatureUrl);
    const hasDeliveryConfirmation = Boolean(order.deliveryConfirmedAt || order.deliveryConfirmedBy);
    ...
    return (
        <article className="rounded-[28px] border border-neutral-200 bg-white p-8 shadow-sm print:rounded-none print:border-0 print:p-10 print:shadow-none">
                ...
                <div className="rounded-[18px] border border-neutral-200 bg-neutral-50 p-4 print:rounded-none print:bg-white print:p-3">
                    ...
                    <dl className="mt-3 space-y-1 text-sm leading-6 text-neutral-700">
                        <div className="flex justify-between gap-4">
                            <dt>Dokument</dt>
                            <dd className="font-medium text-neutral-950">Følgeseddel</dd>
                        </div>
                        <div className="flex justify-between gap-4">
                            <dt>Ordrenummer</dt>
                        ...
                        {hasDeliveryConfirmation ? (
                            <div className="flex justify-between gap-4">
                                <dt>Mottak</dt>
                                <dd className="text-right font-medium text-neutral-950">
                                    {[order.deliveryConfirmedAt, order.deliveryConfirmedBy ? `av ${order.deliveryConfirmedBy}` : null]
                                        .filter(Boolean)
                                        .join(" ")}
                                </dd>
                            </div>
            ...
            <section className="py-8">
                ...
                <div className="mt-4 overflow-hidden rounded-[18px] border border-neutral-200 print:rounded-none">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-neutral-50 text-xs uppercase tracking-[0.12em] text-neutral-500">
                            <tr>
                                ...
                                {cancelledMissingLines ? null : (
                                    <th className="px-4 py-3 text-right font-medium">Bestilt</th>
                                )}
                                <th className="px-4 py-3 text-right font-medium">
                                    {cancelledMissingLines ? "Levert" : "Pakka"}
                                </th>
                                {cancelledMissingLines ? null : (
                                    <th className="px-4 py-3 text-right font-medium">Rest</th>
                                )}
                            </tr>
                        </thead>
                                ...
                                return (
                                    <React.Fragment key={title}>
                                        <tr className="bg-neutral-50">
                                            <td
                                                colSpan={cancelledMissingLines ? 3 : 5}
                                        ...
                                        {lines.map((line) => {
                                            ...
                                            return (
                                                <tr key={`${line.productId}-${line.variantId}`}>
                                                    ...
                                                    <td className="px-4 py-3 text-neutral-600">{line.variantLabel}</td>
                                                    {cancelledMissingLines ? null : (
                ...
                {cancelledMissingLines ? (
                    <p className="mt-4 text-sm leading-6 text-neutral-600">
                        Manglande varer er fjerna etter avtale med kunden. Følgeseddelen viser berre varer som blir levert.
                    </p>
                ) : null}
            </section>

            {showSignatureField ? (
                <section className="grid gap-6 border-t border-neutral-200 pt-8 md:grid-cols-2 print:grid-cols-2">
                    <div className="rounded-[18px] border border-neutral-200 p-4 print:rounded-none print:p-3">
                        <h2 className="text-xs font-medium uppercase tracking-[0.14em] text-neutral-500">
                            Merknad
                        </h2>
            ...
            <footer className="mt-8 border-t border-neutral-200 pt-6 text-xs leading-5 text-neutral-500">
