...
function isAdminUser(user: User | null) {
  ...
  return ADMIN_EMAILS.map((adminEmail) =>
...
function orderNeedsAdminAction(data: any) {
  ...
  const approvalResponse =
    typeof data.approval?.response === "string" ? data.approval.response : "";
  const approvalStatus =
    typeof data.approval?.status === "string" ? data.approval.status : "";
  ...
  const hasCustomerResponse =
    approvalStatus === "answered" || approvalResponse.trim().length > 0;
...
export default function AdminPage() {
  ...
  const [landingSafteriDescription, setLandingSafteriDescription] =
    useState("");
  const [landingBryggeriDescription, setLandingBryggeriDescription] =
    useState("");
  ...
  useEffect(() => listenToCustomers(setCustomers), []);
  ...
  const [geoErrorById, setGeoErrorById] = useState<
  ...
  const [initialOpeningClosedMessage, setInitialOpeningClosedMessage] =
    useState("");
  ...
  const [initialLandingHeroSubtitle, setInitialLandingHeroSubtitle] =
    useState("");
  const [
    initialLandingSafteriDescription,
    setInitialLandingSafteriDescription,
  ] = useState("");
  const [
    initialLandingBryggeriDescription,
    setInitialLandingBryggeriDescription,
  ] = useState("");
  const [initialLandingHeroImages, setInitialLandingHeroImages] = useState<
    string[]
  >([]);
  const [initialLandingHeroWords, setInitialLandingHeroWords] = useState<
    string[]
  >(["", "", ""]);
  ...
  const [initialSafteriVisitDetails, setInitialSafteriVisitDetails] =
    useState("");
  ...
  const [initialSafteriHeroSubtitle, setInitialSafteriHeroSubtitle] =
    useState("");
  const [initialSafteriHeroWords, setInitialSafteriHeroWords] = useState<
    string[]
  >(["", "", ""]);

  const [initialBryggeriHeroKicker, setInitialBryggeriHeroKicker] =
    useState("");
  const [initialBryggeriHeroTitle, setInitialBryggeriHeroTitle] = useState("");
  const [initialBryggeriHeroSubtitle, setInitialBryggeriHeroSubtitle] =
    useState("");
  const [initialBryggeriHeroWords, setInitialBryggeriHeroWords] = useState<
    string[]
  >(["", "", ""]);

  const [initialBryggeriKortTitle, setInitialBryggeriKortTitle] = useState("");
  const [initialBryggeriKortBullets, setInitialBryggeriKortBullets] = useState<
    string[]
  >(["", "", ""]);

  const [initialBryggeriOlDescription, setInitialBryggeriOlDescription] =
    useState("");
  const [initialBryggeriSiderDescription, setInitialBryggeriSiderDescription] =
    useState("");
  ...
  const [initialSafteriKortBullets, setInitialSafteriKortBullets] = useState<
    string[]
  >(["", "", ""]);
  ...
  const [initialSafteriSaftDescription, setInitialSafteriSaftDescription] =
    useState("");
  const [initialSafteriSylteDescription, setInitialSafteriSylteDescription] =
    useState("");
  const [initialSafteriFriskDescription, setInitialSafteriFriskDescription] =
    useState("");
  const [initialSafteriReinDescription, setInitialSafteriReinDescription] =
    useState("");
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, "orders"), (snapshot) => {
      const count = snapshot.docs.filter((orderDoc) =>
        orderNeedsAdminAction(orderDoc.data()),
      ).length;
    ...
    const loadContent = async () => {
      try {
        ...
        if (Array.isArray(data.landingHeroImages)) {
          loadedHeroImages = data.landingHeroImages.filter(
        ...
        if (Array.isArray(data.landingHeroWords)) {
          loadedHeroWords = data.landingHeroWords.filter(
            (v: unknown) => typeof v === "string",
          );
        ...
        if (Array.isArray(data.safteriHeroWords)) {
          loadedSafteriHeroWords = data.safteriHeroWords.filter(
            (v: unknown) => typeof v === "string",
          );
        ...
        if (Array.isArray(data.bryggeriHeroWords)) {
          loadedBryggeriHeroWords = data.bryggeriHeroWords.filter(
            (v: unknown) => typeof v === "string",
          );
        ...
        if (Array.isArray(data.bryggeriKortBullets)) {
          loadedBryggeriKortBullets = data.bryggeriKortBullets.filter(
            (v: unknown) => typeof v === "string",
          );
        ...
        if (Array.isArray(data.safteriKortBullets)) {
          loadedSafteriKortBullets = data.safteriKortBullets.filter(
            (v: unknown) => typeof v === "string",
          );
  ...
  const hasChanges =
    ...
    (openingSeason !== initialOpeningSeason ||
      ...
      JSON.stringify(landingHeroImages) !==
        JSON.stringify(initialLandingHeroImages) ||
      JSON.stringify(landingHeroWords) !==
        JSON.stringify(initialLandingHeroWords) ||
      ...
      JSON.stringify(safteriHeroWords) !==
        JSON.stringify(initialSafteriHeroWords) ||
      // Bryggeri – hero
      ...
      JSON.stringify(bryggeriHeroWords) !==
        JSON.stringify(initialBryggeriHeroWords) ||
      // Safteri – kort fortalt / utval
      safteriKortTitle !== initialSafteriKortTitle ||
      JSON.stringify(safteriKortBullets) !==
        JSON.stringify(initialSafteriKortBullets) ||
      ...
      JSON.stringify(bryggeriKortBullets) !==
        JSON.stringify(initialBryggeriKortBullets) ||
        ...
        <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-4 py-20">
          <div className="rounded-[24px] border border-[color:var(--line)] bg-white/80 p-6 text-center shadow-sm">
            ...
            <p className="mt-3 text-sm leading-6 text-neutral-600">
              Du er logga inn, men denne brukaren har ikkje tilgang til
              administrasjonssida.
            </p>
        ...
        <div className="mx-auto max-w-5xl px-4 py-10 md:py-14">
          <header className="flex flex-col gap-4 border-b border-[color:var(--line)] pb-6 md:flex-row md:items-center md:justify-between">
            ...
            <Link
              href="/"
              target="_blank"
              className="admin-button-secondary inline-flex items-center justify-center px-4 py-2 text-xs"
            >
              Sjå nettsida ↗
            </Link>
          </header>
                  ...
                  <div className="mt-3 space-y-4 text-xs text-neutral-700">
                    ...
                    <label className="flex cursor-pointer items-center gap-2 text-[11px] text-neutral-700">
                      ...
                      <span className="text-neutral-500">
                        (viser melding i staden for åpningstider)
                      </span>
                    </label>
                    ...
                    {!openingClosed && (
                      <div className="space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Fyll inn klokkeslett for dagane de har ope. Tomme felt
                          blir ikkje viste på nettsida.
                        </p>
                    ...
                    {openingClosed && (
                      <div className="space-y-1">
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setOpeningClosedMessage(e.target.value)
                          }
                          className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                          placeholder="Til dømes: Stengt i januar. Opnar igjen i februar."
                        />
                        <p className="text-[11px] text-neutral-500">
                          Denne meldinga blir vist på nettsida i staden for
                          åpningstider.
                        </p>
                      </div>
                    )}
              ...
              <details>
                ...
                <div className="mt-3 space-y-3 text-xs text-neutral-700">
                  ...
                  <p className="mt-1 text-[11px] text-neutral-500">
                    Kontaktinformasjon blir brukt både i besøk-seksjonane og i
                    botnteksten på sida.
                  </p>
          ...
          <section className="mt-6">
            <div className="rounded-[18px] border border-[color:var(--line)] bg-white/70 p-5">
              <details>
                ...
                <div className="mt-3 space-y-3 text-xs text-neutral-700">
                  <p className="text-[11px] text-neutral-500">
                    Her kan du legge inn stader der ein finn produkta våre.
                    Desse blir viste på «Andre utsalsstadar»-sida.
                  </p>
                            ...
                            <button
                              ...
                              onClick={async (e) => {
                                ...
                                const confirmed = await confirmAction({
                                  title: "Slett utsal?",
                                  message: "Utsalet blir fjerna frå nettsida.",
                                  confirmLabel: "Slett utsal",
                                  destructive: true,
                                });
                                if (!confirmed) return;
                                ...
                                await handleSaveContent("outlets", {
                                  outletsOverride: updatedOutlets,
                                });
                              }}
                        ...
                        <div className="mt-3 space-y-3 text-xs text-neutral-700">
                          ...
                          <div className="mt-2 space-y-1">
                            ...
                            <select
                                      ...
                                      ? {
                                          ...
                                          ...(customer
                                            ? {
                                                ...
                                                lat:
                                                  o.lat ?? customer.visitingLat,
                                                lng:
                                                  o.lng ?? customer.visitingLng,
                                              }
                                            : {}),
                                        }
                                      : o,
                              ...
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs"
                            >
                              <option value="">Ingen kundekopling</option>
                            ...
                            <p className="text-[10px] text-neutral-500">
                              Koplinga endrar ikkje kunden. Manglande namn og
                              adresse kan fyllast frå kundekortet.
                            </p>
                          </div>
                          ...
                          <div className="mt-2 space-y-2">
                            ...
                            <p className="text-[11px] text-neutral-500">
                              Skriv til dømes «Nettside», «Facebook» eller
                              «Instagram», og legg inn adressa. Teksten du skriv
                              blir vist som lenkje på nettsida.
                            </p>
                          ...
                          <div className="mt-2 space-y-1">
                            ...
                            <div className="flex flex-col gap-2 md:flex-row md:items-center">
                              ...
                              customers.some(
                                (customer) =>
                                  customer.id === outlet.customerId &&
                                  customer.visitingLat != null &&
                                  customer.visitingLng != null,
                              ) ? (
                                <span className="inline-flex shrink-0 rounded-full bg-emerald-50 px-3 py-1.5 text-[11px] font-medium text-emerald-700">
                                  Koordinatar frå kundekortet
                                </span>
                              ) : (
                                <button
                                  type="button"
                                  onClick={async (e) => {
                                    ...
                                    if (!address) {
                                      setGeoErrorById((prev) => ({
                                        ...prev,
                                        [outlet.id]:
                                          "Skriv inn ei adresse før du hentar koordinatar.",
                                      }));
                                      return;
                                    }
                                    ...
                                    try {
                                      ...
                                      console.error(
                                        "Feil ved henting av koordinatar:",
                                        err,
                                      );
                                      setGeoErrorById((prev) => ({
                                        ...prev,
                                        [outlet.id]:
                                          "Noko gjekk gale ved henting av koordinatar.",
                                      }));
                                    } finally {
                          ...
                          <div className="mt-3 flex items-center justify-between">
                            <button
                              ...
                              disabled={saving || !hasChanges}
                              className={`inline-flex items-center justify-center rounded-full bg-neutral-900 px-4 py-2 text-[11px] text-[color:var(--paper)] disabled:opacity-60 ${
                                !(saving || !hasChanges)
                ...
                <div className="mt-3 space-y-4 text-xs text-neutral-700">
                      ...
                      <div className="mt-3 space-y-3">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setLandingHeroKicker(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setLandingHeroTitle(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setLandingHeroSubtitle(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <p className="text-[11px] text-neutral-500">
                            Kvar boks er eitt ord eller ei kort frase. Desse
                            blir viste med små prikkar mellom på forsida.
                          </p>
                    ...
                    <details>
                      ...
                      <div className="mt-3 space-y-3">
                        <div className="space-y-1">
                          ...
                          <p className="text-[11px] text-neutral-500">
                            Last opp eitt eller fleire bilete. Nettsida vel
                            automatisk eitt per dag. Bileta kan gjerne ha
                            innebygd vannmerke.
                          </p>
                                  ...
                                  try {
                                    ...
                                    for (const file of Array.from(files)) {
                                      try {
                                        ...
                                        console.error(
                                          "Feil ved opplasting av enkeltbilete:",
                                          fileErr,
                                        );
                                      }
                                    }
                                    ...
                                    console.error(
                                      "Feil ved opplasting av hero-bilete:",
                                      err,
                                    );
                                    setHeroUploadError(
                                      "Noko gjekk gale under opplastinga.",
                                    );
                                  } finally {
                                    ...
                                    <button
                                      ...
                                      onClick={async () => {
                                        const confirmed = await confirmAction({
                                          title: "Slett bilete?",
                                          message:
                                            "Biletet blir fjerna frå framsida.",
                                        ...
                                        try {
                                          ...
                                          console.error(
                                            "Feil ved sletting av hero-bilete:",
                                            err,
                                          );
                                        } finally {
                                          setLandingHeroImages((prev) =>
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Tekstane som står under «Safteri» og «Bryggeri» på
                          forsida.
                        </p>
                        <div className="mt-2 space-y-2">
                          <div className="space-y-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setLandingSafteriDescription(e.target.value)
                              }
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setLandingBryggeriDescription(e.target.value)
                              }
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Den korte teksten som står under «Besøk» på forsida
                          (over åpningstidene).
                        </p>
                        <textarea
                          rows={3}
                  ...
                  <button
                    ...
                    disabled={saving || !hasChanges}
                    className={`mt-3 inline-flex items-center justify-center rounded-full bg-neutral-900 px-4 py-2 text-xs text-[color:var(--paper)] disabled:opacity-60 ${
                      !(saving || !hasChanges)
                        ? "hover:bg-neutral-800 hover:-translate-y-0.5 hover:shadow-[0_10px_30px_rgba(0,0,0,0.12)] transition-transform transition-shadow"
                        : ""
                    }`}
                ...
                <div className="mt-3 space-y-4 text-xs text-neutral-700">
                      ...
                      <div className="mt-3 space-y-3">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setSafteriHeroKicker(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setSafteriHeroTitle(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                            placeholder="Skriv tittelen — bruk Enter for ny linje."
                          />
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setSafteriHeroSubtitle(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <p className="text-[11px] text-neutral-500">
                            Kvar boks er eitt ord eller ei kort frase. Desse
                            blir viste med små prikkar mellom på Safteri-sida.
                          </p>
                          <div className="mt-1 flex flex-col gap-2 md:flex-row">
                      ...
                      <div className="mt-3 space-y-3">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setSafteriKortTitle(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <p className="text-[11px] text-neutral-500">
                            Kvar boks er eitt punkt i «Kort fortalt»-lista på
                            Safteri-sida.
                          </p>
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Tekstane som står inne i korta for «Saft», «Sylte og
                          gelé», «Frisk» og «Rein» på Safteri-sida.
                        </p>
                        <div className="mt-2 space-y-2">
                          <div className="space-y-1">
                            <label className="text-[11px] font-medium text-neutral-700">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setSafteriSaftDescription(e.target.value)
                              }
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setSafteriSylteDescription(e.target.value)
                              }
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setSafteriFriskDescription(e.target.value)
                              }
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setSafteriReinDescription(e.target.value)
                              }
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Den korte teksten som står under «Besøk» på
                          Safteri-sida (over butikk og åpningstider).
                        </p>
                        <textarea
                          rows={3}
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Lengre tekst om butikk, besøk og oppleving. Du kan
                          bruke Enter for å lage avsnitt og mellomrom mellom
                          linjer.
                        </p>
                        <textarea
                          rows={6}
                          ...
                          onChange={(e) =>
                            setSafteriVisitDetails(e.target.value)
                          }
                          className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Teksten som står i «Om safteriet»-seksjonen på
                          Safteri-sida.
                        </p>
                        <textarea
                          rows={4}
                ...
                <div className="mt-3 space-y-4 text-xs text-neutral-700">
                      ...
                      <div className="mt-3 space-y-3">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setBryggeriHeroKicker(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setBryggeriHeroTitle(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                            placeholder="Skriv tittelen — bruk Enter for ny linje."
                          />
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setBryggeriHeroSubtitle(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <p className="text-[11px] text-neutral-500">
                            Kvar boks er eitt ord eller ei kort frase. Desse
                            blir viste med små prikkar mellom på Bryggeri-sida.
                          </p>
                          <div className="mt-1 flex flex-col gap-2 md:flex-row">
                      ...
                      <div className="mt-3 space-y-3">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setBryggeriKortTitle(e.target.value)
                            }
                            className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <p className="text-[11px] text-neutral-500">
                            Kvar boks er eitt punkt med tekst. Desse blir viste
                            som korte setningar i «Kort fortalt» på
                            Bryggeri-sida.
                          </p>
                          <div className="mt-1 flex flex-col gap-2">
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Tekstane som står under «Øl» og «Sider» på
                          Bryggeri-sida.
                        </p>
                        ...
                        <div className="space-y-2">
                          <div className="space-y-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setBryggeriOlDescription(e.target.value)
                              }
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                          ...
                          <div className="space-y-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setBryggeriSiderDescription(e.target.value)
                              }
                              className="w-full rounded-[10px] border border-[color:var(--line)] bg-white px-3 py-2 text-xs outline-none placeholder:text-neutral-400 focus:border-neutral-800"
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Teksten som står under «Besøk» på Bryggeri-sida (over
                          åpningstidene).
                        </p>
                        <textarea
                          rows={4}
                    ...
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          ...
                          Du kan bruke Enter for linjeskift, og tom linje for
                          nytt avsnitt.
                        </p>
                  ...
                  <div className="rounded-[14px] border border-[color:var(--line)]/60 bg-white/60 px-3 py-2">
                    <details>
                      ...
                      <div className="mt-3 space-y-2">
                        <p className="text-[11px] text-neutral-500">
                          Teksten som står i «Om bryggeriet»-seksjonen på
                          Bryggeri-sida. Du kan bruke Enter for ny linje, og
                          dobbelt linjeskift for litt ekstra luft.
                        </p>
                        <textarea
                          rows={4}
